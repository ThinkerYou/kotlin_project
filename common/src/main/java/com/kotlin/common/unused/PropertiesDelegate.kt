package com.kotlin.common.unused
...
class PropertiesDelegate(private val path: String) {
   ...
   operator fun<T> getValue(thisRef:Any,property:KProperty<*>):T{
       ...
       var classOfT = property.returnType.classifier as KClass<*>

    return when{
           Boolean::class==classOfT->value.toString().toBoolean()
           Number::class.isSuperclassOf(classOfT)->{
               classOfT.javaObjectType.getMethod("parse${classOfT.simpleName}",String::class.java).invoke(null,value)
           }
            String::class==classOfT ->value
           else ->throw IllegalArgumentException("Not found class type")
       }as T
   }
